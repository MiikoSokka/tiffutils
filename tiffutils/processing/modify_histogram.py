...
def histogram_stretch(input_array, intensity_scaling_param=[1, 99]):
    """
    Apply histogram stretching using percentile clipping to a 3D, 4D, or 5D NumPy array.

    Parameters:
    -----------
    input_array : np.ndarray
        A 3D (Z, Y, X), 4D (Z, C, Y, X), or 5D (T, Z, C, Y, X) NumPy array.
    ...
    Notes:
    ------
    - For 3D arrays, stretching is applied globally across the whole volume.
    ...
    if input_array.ndim == 3:
        print(f'Histogram streching a 3D array of shape {input_array.shape}')
        # 3D: Z, Y, X
        ...
        print(f'Histogram streching a 4D array of shape {input_array.shape}')
        # 4D: Z, C, Y, X
        ...
        print(f'Histogram streching a 5D array of shape {input_array.shape}')
        # 5D: T, Z, C, Y, X
        ...
        for t in range(T):
            for c in range(C):
                ...
                if p99 - p1 < 1e-5:
                    ...
                    scaled = ((clipped - p1) / (p99 - p1)) * (np.iinfo(arr_dtype).max if np.issubdtype(arr_dtype, np.integer) else 1.0)
                    stretched[t, :, c, :, :] = scaled.astype(arr_dtype)

        return stretched

    else:
        raise ValueError("Only 3D (Z,Y,X), 4D (Z,C,Y,X), or 5D (T,Z,C,Y,X) arrays are supported.")