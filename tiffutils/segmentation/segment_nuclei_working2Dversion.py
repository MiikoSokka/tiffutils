...
# ---------------------------------- MODIFYING THIS SECTION ------------------------------------
    ...
    # img_ds: (Z, Y, X) downsampled stack, dtype float32
    Z_ds, H_ds, W_ds = img_ds.shape

    # ---- Cellpose: 2D-only segmentation per slice, no stitching ----
    masks_slices = []

    for z in range(Z_ds):
        img_slice = img_ds[z]  # (H_ds, W_ds), single 2D image

        masks_z, flows_z, styles_z = model.eval(
            img_slice,
            diameter=diameter_eff,
            do_3D=False,         # pure 2D
            stitch_threshold=0,  # absolutely no Cellpose stitching
            min_size=min_size_eff,
            normalize=False,
            batch_size=1,
            channel_axis=None,   # 2D grayscale, no channels
            z_axis=None,         # no z in 2D
        )

        masks_z = np.asarray(masks_z)
        # Safety: if Cellpose returns (1, H, W) for some reason, squeeze it
        if masks_z.ndim == 3:
            masks_z = masks_z[0]

        masks_slices.append(masks_z.astype(np.int32))

    # Stack back to (Z_ds, H_ds, W_ds)
    masks_ds = np.stack(masks_slices, axis=0)

    # ---- Upsample back to original resolution (your existing logic) ----
    # Example sketch – replace with your actual code:
    if downsample_factor != 1:
        # nearest-neighbor upsampling of labels
        from skimage.transform import resize

        Z, H_ds, W_ds = masks_ds.shape
        Z_full, H_full, W_full = vol.shape

        mask_zyx = np.zeros((Z_full, H_full, W_full), dtype=np.int32)
        for z in range(Z_full):
            z_ds = z // downsample_factor
            slice_ds = masks_ds[min(z_ds, Z - 1)]
            mask_zyx[z] = resize(
                slice_ds,
                (H_full, W_full),
                order=0,            # nearest neighbor to keep labels
                preserve_range=True,
                anti_aliasing=False,
            ).astype(np.int32)
    else:
        mask_zyx = masks_ds.astype(np.int32)

    # ---- Our own 3D labeling to get consistent IDs across Z ----
    # 1) Make a binary volume from all non-zero labels
    binary = mask_zyx > 0

    # 2) 3D connected-component labeling:
    #    — treats overlapping voxels across slices as one object
    #    — does NOT merge separate nuclei because CP's boundaries stay zero
    mask_3d = cc_label(binary, connectivity=1)  # (Z, Y, X), int

    # 3) Optional: remove very small 3D objects
    if min_size is not None and min_size > 0:
        mask_3d = remove_small_objects(mask_3d, min_size=min_size)
        mask_3d = np.asarray(mask_3d)  # ensure ndarray after skimage op

    # 4) Return as uint16 to keep label IDs compact
    return mask_3d.astype(np.uint16)
